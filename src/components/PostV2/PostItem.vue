<script>
import {onBeforeUnmount, onBeforeUpdate, onUnmounted, onUpdated, ref} from "vue";

export default{
  emits: ['emit_hashTag', 'emit_like'],
  props: ['item', 'likeLists'],
  setup(props, ctx) {
    let likeBtnStatus = ref(false);
    let itemLike = ref(props.item.like);
    function addLike(id, likes) {
      if (!likeBtnStatus.value) {
        likeBtnStatus.value = true;
        likes++;
        ctx.emit('emit_like', {id: id, like: likes});
        setTimeout(() => {
          likeBtnStatus.value = false;
        }, 0);
      }
    }
    function setHashTag(tag) {
      ctx.emit('emit_hashTag', tag);
    }
    return {
      likeBtnStatus,
      itemLike,
